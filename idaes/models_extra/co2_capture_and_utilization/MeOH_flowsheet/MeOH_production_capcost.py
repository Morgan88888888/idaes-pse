...
__version__ = "customized costing for MeOH production, it is based on the power plant capcost v1.0"
...
class QGESSCostingData(FlowsheetCostingBlockData):
    ...
    def build_process_costs(
        ...
        initial_fills=0,
        waste=None,
        transport_cost_per_tonne_CO2=None,
        tonne_CO2_feedstock=None,
        tonne_CO2_capture=None,
        transport_cost_per_tonne_product=None,
        annual_production_rate=None,
        CE_index_year="2018",
    ):
        """
        ...
        Args:
            ...
            tech: int 1-8 representing the categories in get_PP_costing, used
            ...
            feedstock: string setting feedstock type for feedstock costs
            ...
            initial_fills: Var or value to use for any additional initial fill costs
            waste: string setting waste type for waste costs
            tonne_CO2_capture: Var or value to use for tonnes of CO2 capture
                in one year
            transport_cost_per_tonne_CO2: Expression, Var or Param to use for
                transport costs per tonne of CO2 captured (note, this is not part
                of the TOC)
            tonne_CO2_feedstock: Var or value to use for tonnes of CO2 feedstock
                transported in one year
            annual_production_rate: Var or value to use for annual rate of saleable product
                generated by the plant; relevant for plants where CO2 capture
                is a secondary goal of producing a saleable chemical or mineral
            transport_cost_per_tonne_product: Expression, Var or Param to use for transport
                cost per tonne of product (note, this is not part of the TOC)
        ...
        if fixed_OM and variable_OM:
            ...
            # other owner's costs, financing, spare parts
            if tech == 8:
                # 15% TPC for other owner's costs, 2.7% TPC for financing, 0.5% TPC for spare parts, 2% for preproduction
                self.pct_TPC = Param(
                    initialize=(15 + 2.7 + 0.5 + 2) / 100, doc="Fixed percentage for other owners cost"
                )
            elif tech == 10:
                 # 15% TPC for other owner's costs, 2.7% TPC for financing, 0.5% TPC for spare parts, 2% for preproduction
                self.pct_TPC = Param(
            ...
            if fuel is not None:
                if tech == 8:  # at 100% capacity, 25% of a month
                    self.fuel_cost_OC = Expression(
                        expr=self.variable_operating_costs[0, fuel] / 12 * 0.25 / capacity_factor,
            ...
            if feedstock is not None:
                if tech == 8:  # at 100% capacity, 25% of a month
                    self.feedstock_cost_OC = Expression(
                        expr=self.variable_operating_costs[0, feedstock] / 12 * 0.25 / capacity_factor,
                        doc="Owner's costs - 0.25 months of feedstock costs",
                    )
                elif tech == 10:  
                    self.feedstock_cost_OC = Expression(
                        expr=((self.variable_operating_costs[0, feedstock[0]]+
                              self.variable_operating_costs[0, feedstock[1]] ) / 6 / capacity_factor),
                        doc="EPAT report",
                    )
                else:  # at operating capacity, 2.25 months
                    self.feedstock_cost_OC = Expression(
                        expr=self.variable_operating_costs[0, fuel] / 12 * 0,
                        doc="Owner's costs - power plants don't include feedstock costs",
                    )
                #non_fuel_resources.remove(feedstock)  # remove feedstock from the list

            if waste is not None:
                if tech == 8:  # at 100% capacity, 1 month
            ...
            if chemicals is not None:
                if tech == 8:  # 1392.8 ft3 adsorbent for 6450 kg/hr FA produced
                    if pyunits.get_units(initial_fills) == pyunits.dimensionless:
                        self.chemicals_cost_OC = Expression(
                            expr=(initial_fills * CE_index_units / pyunits.year)
                        )
                    else:  # if units are not currency units, this will fail
                        self.chemicals_cost_OC = Expression(
                            expr=(pyunits.convert(initial_fills, to_units=CE_index_units) / pyunits.year)
                        )
                else:  # six months worth of chemicals for initial fills at operating capacity
                    self.chemicals_cost_OC = Expression(
                        expr=(
            ...
            if chemicals_inventory is not None:
                ...
                if tech == 8:  # at 100% capacity, spare parts already included in other owner's costs
                    ...
                    self.chemicals_inventory_cost_OC = Expression(
                        expr=(
                            ...
                            * pyunits.year  # two months of chemicals inventory
                            + 0.005 * inventory_plant_basis  # inventory spare parts
                        )
                    )

            if tech == 8:  # at 100% capacity
                ...
                self.non_fuel_and_waste_OC = Expression(
                    expr=(
                        sum(self.variable_operating_costs[0, i] for i in resources)
                        / 12 / capacity_factor
                    )
                )
            ...
            if tech == 10:
                ...
                + self.six_month_labor # Initial Cost for Catalyst and Chemicals Inventory
                +(
                (self.maintenance_material_cost / 12 / capacity_factor)  # 1 month materials
                ...
                * self.pct_TPC  # other owners costs (other + spare parts
                ...
                + (
                    (
                        self.maintenance_material_cost / 12 / capacity_factor if tech == 8
            ...
            if tech == 8:
                ...
                self.tasc_toc_factor = Param(
                    initialize=1.047,
                    mutable=True,
                    doc="TASC/TOC factor from S. McNaul, Screening Techno-economic"
            ...
            if tech == 8:
                ...
                self.fixed_charge_factor = Param(
                    initialize=0.0664,
                    mutable=True,
                    doc="Fixed charge rate from S. McNaul, Screening Techno-economic"
                    " Analysis of NETL Reactive Capture Technology, National Energy"
                    " Technology Laboratory, Pittsburgh, September 30, 2022. Exhibit"
                    " 4-8",
                )
            ...
            self.additional_cost_of_electricity.fix()
            ...
            if tonne_CO2_feedstock is not None:
                if not hasattr(self, "tonne_CO2_feedstock"):
                    self.tonne_CO2_feedstock = Param(
                        initialize=tonne_CO2_feedstock, mutable=True, units=pyunits.tonne
                    )
            ...
            if hasattr(self, "tonne_CO2_capture"):
            ...
            if transport_cost_per_tonne_CO2() is not None:

                # except the case where transport_cost_per_tonne_CO2 is passed but
                # tonne_CO2_capture and tonne_CO2_feedstock are not passed
                if tonne_CO2_capture is None and tonne_CO2_feedstock is None:
                    # PYLINT-TODO
                    # pylint: disable-next=broad-exception-raised
                    raise Exception(
                        "If a transport_cost_per_tonne_CO2 is not None, "
                        "tonne_CO2_capture and tonne_CO2_feedstock cannot "
                        "both be None."
                    )

                if (
                    isinstance(
                        transport_cost_per_tonne_CO2,
                        (Expression, ScalarExpression, Param, Var),
                    )
                    and pyunits.get_units(transport_cost_per_tonne_CO2)
                    == pyunits.dimensionless
                ) or isinstance(transport_cost_per_tonne_CO2, (int, float)):
                    self.CO2_transport_cost = (
                        transport_cost_per_tonne_CO2
                        * CE_index_units * 1e-6
                        / pyunits.tonne
                        * (
                            (
                                self.tonne_CO2_feedstock if tonne_CO2_feedstock is not None
                                else 0 * pyunits.tonne/pyunits.year
                                )
                            +
                            (
                                self.tonne_CO2_capture if tonne_CO2_capture is not None
                                else 0 * pyunits.tonne/pyunits.year
                                )
                            )
                        )
                else:
                    self.CO2_transport_cost = pyunits.convert(
                        transport_cost_per_tonne_CO2 *
                        (
                            (
                                self.tonne_CO2_feedstock if tonne_CO2_feedstock is not None
            ...
            # build cost of production (COP)
            if annual_production_rate is not None:
                if tech == 8:
                    self.additional_cost_of_production = Var(
                        initialize=0,
                        doc="Additional cost to be added to the COP calculations"
                        + " in millions",
                        units=getattr(pyunits, "MUSD_" + CE_index_year) / pyunits.kg,
                    )
                    # assume the user is not using this
                    self.additional_cost_of_production.fix(0)

                    if not hasattr(self, "annual_production_rate"):
                        if (
                            pyunits.get_units(annual_production_rate)
                            == pyunits.dimensionless
                        ):
                            raise UnitsError(
                                "The argument annual_production_rate was passed as a dimensionless "
                                "quantity with no units. Please ensure that the feed "
                                "rate is passed in units of mass / time."
                            )
                        else:  # use source units
                            self.annual_production_rate = Param(
                                initialize=annual_production_rate,
                                mutable=True,
                                units=pyunits.get_units(annual_production_rate),
                            )

                    annual_production_rate_units = pyunits.get_units(self.annual_production_rate)

                    # check that units are compatible
                    try:
                        pyunits.convert(
                            self.annual_production_rate,
                            to_units=pyunits.kg / pyunits.year,
                        )
                    except InconsistentUnitsError:
                        raise UnitsError(
                            f"The argument annual_production_rate was passed with units of "
                            f"{annual_production_rate_units} which cannot be converted "
                            f"to units of mass per year. Please ensure that annual_production_rate "
                            "is passed with rate units of mass per year (mass/a) or dimensionless."
                        )

                    # check that units are on an annual basis
                    if str(annual_production_rate_units).split("/")[1] not in ["a", "year"]:
                        raise UnitsError(
                            f"The argument annual_production_rate was passed with units of "
                            f"{annual_production_rate_units} and must be on an anuual basis. Please "
                            f"ensure that annual_production_rate is passed with rate units "
                    ...
                    self.cost_of_production = Expression(
                        expr=(
                            pyunits.convert(
                                ...
                                / (self.annual_production_rate * capacity_factor),
                                to_units=CE_index_units/pyunits.kg,
                            )
                            + self.additional_cost_of_production
                        )
                    )

                    if transport_cost_per_tonne_product is not None:
                        if (
                            isinstance(
                                transport_cost_per_tonne_product,
                                (Expression, ScalarExpression, Param, Var),
                            )
                            and pyunits.get_units(transport_cost_per_tonne_product)
                            == pyunits.dimensionless
                        ) or isinstance(transport_cost_per_tonne_product, (int, float)):
                            self.transport_cost_of_product = (
                                transport_cost_per_tonne_product
                                * CE_index_units * 1e-6
                                / pyunits.tonne
                                * pyunits.convert(
                            ...
                            self.transport_cost_of_product = pyunits.convert(
                                transport_cost_per_tonne_product
                                * self.annual_production_rate,
                                to_units=CE_index_units / pyunits.year,
                            )
                    ...
                    self.additional_cost_of_production = Var(
                        ...
                        doc="Additional cost to be added to the COP calculations"
                        + " in millions",
                        units=getattr(pyunits, "MUSD_" + CE_index_year) / pyunits.gal,
                    )
                    # assume the user is not using this
                    self.additional_cost_of_production.fix(0)
                    ...
                    try:
                        pyunits.convert(
                            self.annual_production_rate,
                            to_units=pyunits.gal / pyunits.year,
                        )
                    except InconsistentUnitsError:
                        raise UnitsError(
                            f"The argument annual_production_rate was passed with units of "
                    ...
                    self.cost_of_production = Expression(
                        expr=(
                            pyunits.convert(
                                ...
                                / (self.annual_production_rate * capacity_factor),
                                to_units=CE_index_units/pyunits.gal,
                            )
                            + self.additional_cost_of_production
                        )
                    )

            else:  # except the case where transport_cost_per_tonne_product is passed but annual_production_rate is not passed
                if transport_cost_per_tonne_product is not None:
                    raise AttributeError(
                        "If transport_cost_per_tonne_product is not None, "
                        "annual_production_rate cannot be None."
                    )
    ...
    def report(self, export=False):
        ...
        if hasattr(self, "cost_of_production"):
            var_dict["Cost of Production [$/kg product]"] = value(
                self.cost_of_production * 1e6
            )

        if hasattr(self, "transport_cost_of_product"):
            var_dict["Total Transport Cost Of Product [$MM]"] = value(self.transport_cost_of_product)

        if hasattr(self, "cost_of_capture"):
            var_dict["Cost of Capture [$/tonne CO2]"] = value(self.cost_of_capture * 1e6)

        if hasattr(self, "CO2_transport_cost"):
            var_dict["Total Transport Cost of Feedstock & Captured CO2 [$MM]"] = value(self.CO2_transport_cost)
    ...
    def get_PP_costing(
        ...
        Args:
            ...
            tech: integer 1-8 representing the above categories
        ...
        if multiply_project_conting is True:
            ...
            def total_plant_cost_rule(blk, i):
                return blk.total_plant_cost[i] == blk.bare_erected_cost[i] * (
                    1 + blk.eng_fee[i] + blk.process_conting[i] + blk.project_conting[i]
                )
    ...
    def get_fixed_OM_costs(
        ...
        b.maintenance_material_percent = Param(
            initialize=maintenance_percentages[tech][2], mutable=True
        )
    ...
    def get_variable_OM_costs(
        ...
        default_prices = {
            ...
            "amine_purification_unit_waste": 38 * 1e-6 * CE_index_units / pyunits.ton,
        ...
        if not set(resources).issubset(default_prices.keys()):
            missing_prices = [i for i in resources if i not in prices.keys()]
            # PYLINT-TODO
            # pylint: disable-next=broad-exception-raised
            raise Exception(
                f"{missing_prices} do not have prices listed in the price "
                "dictionary. Prices exist for the following resources: "
    ...
    def get_total_TPC(b, CE_index_year):
        ...
        @b.Constraint()
        def total_TPC_eq(c):
            return c.total_TPC == sum(TPC_list) +0.026 # addded vacuum pump
